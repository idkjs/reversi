...
describe("Board", () => {
    ...
    describe("getCountForColor", () => {
        test("should return the number of cells of corresponding color", () => Board.({
            expect(getCountForColor(init(4, 4), Some(Cell.Black))) |> toBe(2)|>ignore;
            expect(getCountForColor(init(4, 4), Some(Cell.White))) |> toBe(2)|>ignore;
    ...
    describe("isOutOfBound", () => {
        ...
        test("should return true for out of bounds coords", () => {
            expect(Board.isOutOfBound(baseBoard, (3, 4))) |> toBe(true)|>ignore;
            expect(Board.isOutOfBound(baseBoard, (4, 3))) |> toBe(true)|>ignore;
            expect(Board.isOutOfBound(baseBoard, (-1, 0))) |> toBe(true)|>ignore;
            expect(Board.isOutOfBound(baseBoard, (0, -1))) |> toBe(true);
        });

        test("should return false for inner bounds coords", () => {
            expect(Board.isOutOfBound(baseBoard, (0, 0))) |> toBe(false)|>ignore;
            expect(Board.isOutOfBound(baseBoard, (3, 3))) |> toBe(false)|>ignore;
